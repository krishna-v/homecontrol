...
function relayKNXCmd(room_id, ctrl_id, key, value) {
	logMessage("INFO", "relayKNXCmd: Turning " + (value === "1" ? "on " : "off ") +
		model.room(room_id).name + " " + model.control(room_id, ctrl_id).name);
...
function relayTasmotaCmd(room_id, ctrl_id, key, value) {
	const host = model.control(room_id, ctrl_id).host;	
    const name = model.room(room_id).name + " " + model.control(room_id, ctrl_id).name;
	let cmdstr = '';
...
function loadTasmotaState(room, ctrl, host) {
	_doRemoteCmd(host, 80, '/cm?cmnd=Status%2011').then((data) => {
		try {
			...
			model.setControlState(room, ctrl, { "state": state, "color": color });
			hbe.setState(model.control(room, ctrl).hueid, hueState);
		} catch(e) {
		...
		if(e.code === 'EHOSTUNREACH') {
			 // host offline.
			if(model.control(room, ctrl).state !== "offline") {
				logMessage("WARN", 'loadTasmotaState: ' + e.message + ' for ' + host);
				model.setControlState(room, ctrl, { "state": "offline" });
...
function loadKNXState(room, ctrl, host) {
	...
	_doRemoteCmd(host, 80, arg).then((data) => {;
		...
		model.setControlState(room, ctrl, { "state": state });
        hbe.setState(model.control(room, ctrl).hueid, { reachable: true, on: (state === "on") });
	}, (e) => { 
		if(e.code === 'EHOSTUNREACH') {
			 // host offline.
